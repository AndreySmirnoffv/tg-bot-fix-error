...
import yooKassaData from './yooKassa.json' with {type: "json"}
...
export const yooController = new YooModule(process.env.SHOP_ID, process.env.SECRET_KEY)
console.log(process.env.SHOP_ID); // Должен быть числовым идентификатором магазина
console.log(process.env.SECRET_KEY); // Должен быть строкой с секретным ключом
...
bot.action(/subPayConfirm/, async (ctx) => {
    try {
        ...
        await bot.telegram.sendMessage(`${Number(information.channel)}`, notify).catch(err => { console.log(err) })
        main_keyboard(ctx)
    } catch (e) {
...
// 		}
// 	};
// 	let payment = await yooController.createPayment(createPayload);
//     console.log(payment)
// 	return payment;
// }
...
bot.action(/subPay/, async (ctx) => {
    ...
    try {
        ...
        // Generate idempotence key for the payment process (prevents duplicate payments)
        let idempotenceKey = uuid();
        console.log('Idempotence Key:', idempotenceKey);

        // Validate that there is a match and input
        if (!ctx.match || !ctx.match.input) {
            console.log('No match or input found');
        ...
        if (!args[1]) {
            ...
            return ctx.reply('Invalid subscription index.');
        }
        ...
        if (!subs[idx]) {
            console.log('Invalid subscription index');
        ...
        console.log('Subscription Price:', summ_pay);

        const payload = {
            amount: summ_pay,
            currency: "RUB",
            paymentType: "bank_card",
            returnUrl: yooKassaData.redirect_url
        };

        try {
            console.log("метод вызывался")
            const payment = await new Payment().topUpBalance(bot, chatId, payload, user);
            console.log(payment)
            // let userReferal = await $user.findOne({ id: payment.metadata.telegramUserId });
            console.log("Метод отработал")
        } catch (error) {
            throw error
        }

        // if (userReferal.referalId) {
        ...
        await ctx.reply('⚠️ Произошла ошибка при выставлении счёта. Пожалуйста, попробуйте снова.');
    }
